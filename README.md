...
`Docker BuildKit` est un moteur de build nouvelle génération pour `Docker` introduit à partir de `Docker 18.09` et devenu le builder par défaut depuis `Docker 23.0`. Outre la paralélisation et une optimisation du cache, il introduit les montages secrets (`--mount=type=secret`). Cette fonctionnalité permet d'accéder à un secret uniquement pendant l'exécution d'une instruction `RUN`, sans que ce secret ne soit jamais stocké dans un layer de l'image.

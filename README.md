...
### Les ports privilégiés

Sur les systèmes Linux, seul l'utilisateur `root` utiliser les ports inférieurs à `1024` (ports privilégiés). Un utilisateur non-root ne peut pas écouter sur le port `80` ou `443`.

Pour cette raison, les images `.NET 8` écoutent par défaut sur le port `8080`, ce qui est compatible avec une exécution non-root.
...
* Essayer de lier le conteneur non-root au port `80` au lieu de `8080` et observer l'erreur
